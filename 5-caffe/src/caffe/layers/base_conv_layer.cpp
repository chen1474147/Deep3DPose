...
class CVHandler: public Listener {
public:
    CVHandler(bool *is_grl_train) :
...
void BaseConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    // parameter
    if (this->layer_param_.convolution_param().has_is_grl_layer()) {
        is_grl_layer = this->layer_param_.convolution_param().is_grl_layer();
    } else {
        // normal layer
        is_grl_layer = 0;
    }
    ...
    if (is_grl_layer == 0) {
        LOG(INFO) << "This ConvolutionLayer is normal layer";
    } else if (is_grl_layer == 1) {
        LOG(INFO) << "This ConvolutionLayer is grl layer";
    } else if (is_grl_layer == 2) {
        LOG(INFO) << "This ConvolutionLayer is NOT grl layer";
    } else {
        LOG(FATAL) << "This ConvolutionLayer is unknown layer";
    }

    is_grl_train = false;

    Messenger::AddListener("IS_TRAIN_GRL_CHANGED",
            new CVHandler(&is_grl_train));
}
...
void BaseConvolutionLayer<Dtype>::forward_cpu_gemm(const Dtype* input,
    ...
    for (int g = 0; g < group_; ++g) {
        caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,
                ...
                kernel_dim_ / group_, (Dtype) 1., weights + weight_offset_ * g,
...
void BaseConvolutionLayer<Dtype>::backward_cpu_bias(Dtype* bias,
    ...
    caffe_cpu_gemv<Dtype>(CblasNoTrans, num_output_, height_out_ * width_out_,
            1., input, bias_multiplier_.cpu_data(), 1., bias);
}
...
void BaseConvolutionLayer<Dtype>::backward_gpu_bias(Dtype* bias,
    ...
    caffe_gpu_gemv<Dtype>(CblasNoTrans, num_output_, height_out_ * width_out_,
            1., input, bias_multiplier_.gpu_data(), 1., bias);
}

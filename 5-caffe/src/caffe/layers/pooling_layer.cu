...
__global__ void MaxPoolForward(const int nthreads, const Dtype* bottom_data,
        ...
        const int pad_h, const int pad_w, Dtype* top_data, int* mask,
        Dtype* top_mask) {
...
__global__ void StoPoolForwardTrain(const int nthreads,
        ...
        const int stride_h, const int stride_w, Dtype* rand_idx,
        Dtype* top_data) {
    CUDA_KERNEL_LOOP(index, nthreads)
    {
        int pw = index % pooled_width;
        ...
        for (int h = hstart; h < hend; ++h) {
            for (int w = wstart; w < wend; ++w) {
                cumsum += bottom_data[h * width + w];
                if (cumsum >= thres) {
                    rand_idx[index] = ((n * channels + c) * height + h) * width
                            + w;
...
__global__ void StoPoolForwardTest(const int nthreads, const Dtype* bottom_data,
    ...
    CUDA_KERNEL_LOOP(index, nthreads)
    {
        ...
        for (int h = hstart; h < hend; ++h) {
            for (int w = wstart; w < wend; ++w) {
                cumsum += bottom_data[h * width + w];
                cumvalues += bottom_data[h * width + w]
...
void PoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    case PoolingParameter_PoolMethod_MAX:
        ...
        MaxPoolForward<Dtype> <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
                ...
                stride_h_, stride_w_, pad_h_, pad_w_, top_data, mask, top_mask);
        break;
    case PoolingParameter_PoolMethod_AVE:
        ...
        AvePoolForward<Dtype> <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
                count, bottom_data, bottom[0]->num(), channels_, height_,
                width_, pooled_height_, pooled_width_, kernel_h_, kernel_w_,
                stride_h_, stride_w_, pad_h_, pad_w_, top_data);
        break;
    case PoolingParameter_PoolMethod_STOCHASTIC:
        if (this->phase_ == TRAIN) {
            ...
            StoPoolForwardTrain<Dtype> <<<CAFFE_GET_BLOCKS(count),
                    ...
                    stride_h_, stride_w_, rand_idx_.mutable_gpu_data(),
                    top_data);
        } else {
            // NOLINT_NEXT_LINE(whitespace/operators)
...
template<typename Dtype>
__global__ void StoPoolBackward(const int nthreads, const Dtype* rand_idx,
        const Dtype* top_diff, const int num, const int channels,
        const int height, const int width, const int pooled_height,
        const int pooled_width, const int kernel_h, const int kernel_w,
        const int stride_h, const int stride_w, Dtype* bottom_diff) {
    CUDA_KERNEL_LOOP(index, nthreads)
    {
        // find out the local index
        // find out the local offset
        ...
        for (int ph = phstart; ph < phend; ++ph) {
            for (int pw = pwstart; pw < pwend; ++pw) {
                gradient += top_diff[ph * pooled_width + pw]
                        * (index
                                == static_cast<int>(rand_idx[ph * pooled_width
                                        + pw]));
            }
        }
        bottom_diff[index] = gradient;
...
void PoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    case PoolingParameter_PoolMethod_STOCHASTIC:
        ...
        StoPoolBackward<Dtype> <<<CAFFE_GET_BLOCKS(count),
                CAFFE_CUDA_NUM_THREADS>>>(count, rand_idx_.gpu_data(), top_diff,
                top[0]->num(), channels_, height_, width_, pooled_height_,
                pooled_width_, kernel_h_, kernel_w_, stride_h_, stride_w_,
                bottom_diff);
        break;

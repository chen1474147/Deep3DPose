...
void Solver<Dtype>::Init(const SolverParameter& param) {
    ...
    // set grl and fc parameters
    // set grl train
    if (param_.has_grl_interval()
            && param_.has_fc_interval()
            && param_.has_grl_train()) {
        grl_interval = param_.grl_interval();
        fc_interval = param_.fc_interval();
        grl_train = param_.grl_train();
        LOG(INFO) << "grl_interval: " << grl_interval;
        LOG(INFO) << "fc_interval: " << fc_interval;
        LOG(INFO) << "grl_train: " << grl_train;
    } else {
        LOG(FATAL) << "No grl and fc interval parameters!";
    }
}
...
void Solver<Dtype>::InitTestNets() {
    ...
    // are evaluated.
    const int num_generic_net_instances = param_.test_iter_size()
            - num_test_nets;
    const int num_test_net_instances = num_test_nets
...
void Solver<Dtype>::Step(int iters) {
    ...
    int sum_interval = grl_interval + fc_interval;
    for (; iter_ < stop_iter; ++iter_) {
        Messenger::SendMessage("SOLVER_ITER_CHANGED", &iter_);

        // first, we train a good grl
        // then, we do a round train
        if (iter_ < grl_train) {
            bool is_grl_train = true;
            Messenger::SendMessage("IS_TRAIN_GRL_CHANGED", &is_grl_train);
        } else {
            // first, we will train grl
            // then, we will train regression
            // we do it in a round way
            int now_interval = iter_ % sum_interval;
            if (now_interval < grl_interval) {
                bool is_grl_train = true;
                Messenger::SendMessage("IS_TRAIN_GRL_CHANGED", &is_grl_train);
                // LOG(INFO)<< "Training domain adaptations";
            } else {
                bool is_grl_train = false;
                Messenger::SendMessage("IS_TRAIN_GRL_CHANGED", &is_grl_train);
                // LOG(INFO) << "Training regressions";
            }
        }
